...
class AgentActivity(RecognitionHooks):
    ...
    async def start(self) -> None:
        ...
        async with self._lock:
            ...
            try:
                ...
                # logger.info("[KWS DEBUG] AgentActivity.start() called")
    ...
    async def _start_session(self) -> None:
        # logger.info("[KWS DEBUG] _start_session called")
    ...
    async def _close_session(self) -> None:
        ...
        # kws 控制
        if self._kws_task is not None:
             self._kws_task.cancel()
             try:
                 await self._kws_task
    ...
    async def _run_kws(self):
        ...
        try:
            ...
            # logger.info(f"[KWS DEBUG] Connecting to KWS at {uri}")
            async with websockets.connect(uri) as ws:
                # logger.info("[KWS DEBUG] KWS Connected Successfully")
                
                async def send_loop():
                    # logger.info("[KWS DEBUG] send_loop started")
                    while True:
                        data = await self._kws_queue.get()
                        # logger.info(f"[KWS DEBUG] Got audio chunk, size: {len(data)}")
                        try:
                            await ws.send(data)
                        except Exception as e:
                            logger.error(f"KWS Send Error: {e}")
                            break
                    ...
                    while True:
                        try:
                            ...
                            if msg_type == "wake_detected":
                                ...
                                logger.info(f"[kws] 服务端就绪：{response.get('message')}")
                        except Exception as e:
                            logger.error(f"KWS Recv Error: {e}")
                            break
                            
                await asyncio.gather(send_loop(), recv_loop())
        except Exception as e:
            logger.error(f"KWS Connection Error: {e}")

    def _on_user_state_changed(self, event: UserStateChangedEvent):
        # 该函数是用户状态变为away，系统进入休眠时触发
        if self.is_awake and event.new_state == "away":
            logger.info("\n用户状态变为了away\n")
