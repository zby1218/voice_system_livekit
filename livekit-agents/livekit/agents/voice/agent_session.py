...
class AgentSession(rtc.EventEmitter[EventTypes], Generic[Userdata_T]):
    ...
    async def _forward_audio_task(self) -> None:
        ...
        if audio_input is None:
            # logger.error("❌ [Debug] self.input.audio 是 None！")
            return
        # logger.info("✅ [Debug] 开始转发音频任务 (_forward_audio_task)")
        ...
        async for frame in audio_input:
            # 调试
            packet_count += 1
            # 每收 50 帧打印一次，证明数据还在流动
            # if packet_count % 50 == 0:
                # logger.info(f"🌊 [Debug] Session收到音频流 | 累计帧数: {packet_count}")
            
            if self._activity is not None:
                self._activity.push_audio(frame)
            else:
                logger.warning("⚠️ [Debug] self._activity 是 None，音频被丢弃！")
            # 调试结束
