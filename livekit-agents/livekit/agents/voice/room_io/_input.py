...
class _ParticipantInputStream(Generic[T], ABC):
    ...
    def _on_track_available(
        ...
        # logger.info(
        #     f"🕵️ [Input] 收到音轨申请 | Track: {track.sid} | "
        #     f"用户: {participant.identity} (目标: {self._participant_identity}) | "
        #     f"类型: {rtc.TrackSource.Name(publication.source)} | "
        #     f"允许列表: {[rtc.TrackSource.Name(s) for s in self._accepted_sources]}"
        # )
        ...
        if (
            ...
            if self._participant_identity != participant.identity:
                logger.warning(f"🚫 [Input] 拒绝: 用户身份不匹配")
            elif publication.source not in self._accepted_sources:
                logger.warning(f"🚫 [Input] 拒绝: 音轨类型 ({rtc.TrackSource.Name(publication.source)}) 不在允许列表中!")
            return False
