...
class _ParticipantAudioOutput(io.AudioOutput):
    ...
    async def _publish_track(self) -> None:
        ...
        async with self._lock:
            ...
            # logger.info(f"📡 [RoomIO] 音频轨道已发布并订阅成功! SID: {self._publication.sid}")
    ...
    async def _forward_audio(self) -> None:
        # logger.info("🟢 [RoomIO] _forward_audio loop started")
        async for frame in self._audio_buf:
            # logger.info(f"🔄 [RoomIO] _forward_audio loop iter | pushed_dur: {self._pushed_duration} | interrupted: {self._interrupted_event.is_set()}")
            if not self._playback_enabled.is_set():
                # logger.info("⏸️ [RoomIO] _forward_audio waiting for playback_enabled")
                self._audio_source.clear_queue()
                await self._playback_enabled.wait()
