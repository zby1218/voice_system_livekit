...
class Qwen2Encoder(torch.nn.Module):
    ...
    def forward_one_step(self, xs, masks, cache=None):
        ...
        # print("="*80)
        # print(f"[DEBUG] Type of outs: {type(outs)}")
        # print(f"[DEBUG] outs has hidden_states attr: {hasattr(outs, 'hidden_states')}")
        # print(f"[DEBUG] outs.hidden_states is None: {outs.hidden_states is None}")
        
        # if outs.hidden_states is not None:
        #     print(f"[DEBUG] Type of hidden_states: {type(outs.hidden_states)}")
        #     print(f"[DEBUG] Length of hidden_states tuple: {len(outs.hidden_states)}")
        #     print(f"[DEBUG] Shape of first hidden_state: {outs.hidden_states[0].shape}")
        #     print(f"[DEBUG] Shape of last hidden_state: {outs.hidden_states[-1].shape}")
        # else:
        #     print("[DEBUG] hidden_states is None!")
        #     print(f"[DEBUG] Available keys in outs: {list(outs.keys()) if hasattr(outs, 'keys') else 'N/A'}")
            
        #     # 检查是否有 last_hidden_state
        #     if hasattr(outs, 'last_hidden_state'):
        #         print(f"[DEBUG] last_hidden_state available: {outs.last_hidden_state is not None}")
        #         if outs.last_hidden_state is not None:
        #             print(f"[DEBUG] Shape of last_hidden_state: {outs.last_hidden_state.shape}")
        
        # print("="*80)
        
        # # 根据情况选择正确的 hidden state
        # if outs.hidden_states is not None and len(outs.hidden_states) > 0:
        #     xs = outs.hidden_states[-1]
        # elif hasattr(outs, 'last_hidden_state') and outs.last_hidden_state is not None:
        #     print("[WARNING] Using last_hidden_state as fallback!")
        #     xs = outs.last_hidden_state
        # else:
        #     raise ValueError(
        #         f"Cannot extract hidden states! "
        #         f"hidden_states is None: {outs.hidden_states is None}, "
        #         f"has last_hidden_state: {hasattr(outs, 'last_hidden_state')}"
        #     )


        # Hack: 手动获取最后一层的输出
        # 这需要直接访问模型的 transformer 层
        # 对于 Qwen 模型，结构通常是 model.transformer.h[-1] 或 model.model.layers[-1]
